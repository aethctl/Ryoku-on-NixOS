...
- **Never edit a shipped file in place.** The packaged Ryoku base (for example
  `/usr/share/ryoku/` on Arch or an immutable Nix store path on NixOS) and the
  files Ryoku lays into `~/.config/quickshell/` are re-laid on
