...
// hyprplugins.go: the Hyprland compositor plugins the Hub manages. A plugin is
// one .so Hyprland dlopens, ABI-locked to the exact compositor build (commit +
// the major.minor of aquamarine, hyprutils, hyprgraphics, hyprcursor and
// hyprlang). Three places can hold a copy: the [ryoku] package under pluginDir,
// a local build under userPluginDir (deploy.sh on a checkout, the Hub's Rebuild
// and Add on any box), and hyprpm's own tree for plugins a user installed by
// hand. Beside every copy Ryoku builds sits a `<id>.abi` receipt naming the ABI
// it was compiled against, so the Hub, the doctor and settings.lua can tell a
// stale build from a working one without loading it.
//
// `ryoku-hub hypr plugins list` reports every plugin with its source, version,
// ABI verdict and detected settings; rebuild/add/remove live in
// hyprplugins_build.go.
...
// pluginDef is one plugin Ryoku bundles: shipped as a [ryoku] package, exposed
// on the Hub's Plugins page with curated settings, and rebuildable here from the
// same upstream the package builds from.
type pluginDef struct {
	ID      string // the .so basename, the Hub id and the receipt name
	Name    string // what the Plugins page calls it
	Desc    string
	Repo    string // upstream git URL; "" for a plugin authored in this repo
	Plugin  string // the [table] in the repo's hyprpm.toml
	Local   string // in-tree source dir (relative to a checkout) for a Ryoku-authored plugin
	Package string // the [ryoku] pacman package that ships the .so
	Docs    string // where the plugin's own settings are documented
	Sounds  string // for a plugin that ships samples: where they come from
...
// keysoundsUserDir is where the user's own key sound profiles go (the plugin
// looks there first). Created on demand with a README that says what a
// profile is and how to import a Mechvibes pack, so the Hub's "My sounds"
// opens a folder that explains itself rather than an empty one.
...
// keysoundsShippedDir is where the installed profiles are: beside a local
// build of the plugin when there is one, else the package's.
func keysoundsShippedDir() string {
	if nixManagedHyprPlugins() {
		shipped := filepath.Join(
			pluginPackageDir(),
			"keysounds",
		)
...
// nixManagedHyprPlugins is explicit rather than inferred from /etc/NIXOS.
// This keeps upstream unit tests and development checkouts portable while the
// NixOS module can opt the installed desktop into immutable plugin ownership.
func nixManagedHyprPlugins() bool {
	return strings.EqualFold(
		strings.TrimSpace(
			os.Getenv(
				"RYOKU_HYPR_PLUGINS_MANAGED",
			),
		),
		"nix",
	)
...
// pluginPackageDir resolves the immutable package tier. Arch keeps using the
// FHS path; NixOS injects the exact store path for the active Ryoku generation.
...
// hyprpmDir is hyprpm's own plugin tree, discovered read-only so a plugin the
// user installed by hand still shows on the Plugins page.
...
// major.minor of each library the plugin API leaks through its headers. The
// string form is exactly `hyprctl version -j`'s abiHash and what
// __hyprland_api_get_client_hash bakes into a plugin.
type hyprABI struct {
...
// abiDiff names what moved between the ABI a plugin was built for and the one
// the compositor runs, in words a user can act on ("built for aquamarine 0.14,
// running 0.15"). Empty when they agree.
...
type compositorInfo struct {
	...
	ABI     string  `json:"abi"`
	Live    bool    `json:"live"`    // a compositor answered
	Headers string  `json:"headers"` // the ABI a build against the installed headers yields
	abi     hyprABI // parsed ABI
}

// liveCompositor asks the running compositor for its version and ABI hash,
// once per process: settings.lua generation resolves every plugin against it.
...
// headersVersionPath locates Hyprland's version.h, the file a plugin build
// bakes its ABI from.
...
// produces, from version.h's macros, with the same formula the plugin API uses.
// This is the build target: a receipt equal to it means rebuilding changes
...
func headersABI() (hyprABI, string) {
	if nixManagedHyprPlugins() {
		...
		if err == nil {
			return parseABI(string(b)), "NixOS"
		}
	}
...
type soCopy struct {
	Path string
	Tier string // "built" (userPluginDir), "package" (pluginDir), "hyprpm"
	ABI  hyprABI
}

// pluginCopies lists every copy of a plugin, local build first: a checkout
// loads what deploy.sh just built, and a box that rebuilt a stale package copy
// loads the fresh one.
func pluginCopies(id string) []soCopy {
	...
	// Bundled plugins are generation-owned on NixOS. A stale local .so from an
	// older deployment must never shadow the compositor-matched package copy.
	if !nixManagedHyprPlugins() || !bundled {
		candidates = append(
			candidates,
			soCopy{
				Path: filepath.Join(
	...
	// A bundled Nix plugin has one authoritative source: the generation.
	if nixManagedHyprPlugins() && bundled {
		return out
	}

	for _, p := range hyprpmCopies() {
		if strings.TrimSuffix(
			filepath.Base(p),
			".so",
		) == id {
			out = append(
...
// pickCopy chooses the copy settings.lua should load for a target ABI: the
// first whose receipt matches, else the first with no receipt (an unknown build
// gets the benefit of the doubt, as every copy did before receipts existed).
// stale is true when every copy carries a receipt and none matches, so the
// caller can leave the load out rather than have Hyprland refuse it on every
// reload.
...
// right now, by the name each reports (the path it came from is not exposed).
func loadedPlugins() map[string]loadedPlugin {
	out, err := exec.Command("hyprctl", "plugins", "list", "-j").Output()
...
// hyprctlPluginLoad asks the compositor to load a .so; the error text is
// Hyprland's own verdict ("version mismatch, built against: ..., running
// compositor: ...").
func hyprctlPluginLoad(path string) error {
	out, err := exec.Command("hyprctl", "plugin", "load", path).CombinedOutput()
...
func hyprctlPluginUnload(path string) {
	_ = exec.Command("hyprctl", "plugin", "unload", path).Run()
}

// unloadPluginCopies drops every copy of a plugin the compositor might hold.
// A reload only unloads a plugin it loaded from config itself; one the roster
// probe or loadEnabledPlugins loaded by hand stays, so a Save that turns one
...
// after a reload. Hyprland compares the declared path set with the last one
// and does nothing when it is unchanged, so a plugin unloaded by hand (a
// rebuild, a Save that turned it off then on) never comes back through the
// config alone. Returns true when something was loaded, so the caller can push
...
// .so (`plugin:<ns>:...` literals) and typed by the compositor once the plugin
// is loaded (`hyprctl getoption`). Key drops the `plugin:` prefix.
type detectedSetting struct {
	Key     string `json:"key"`
	Type    string `json:"type,omitempty"` // bool | int | float | str | color | custom
...
// value it currently holds (the default while nothing set it).
func typeSetting(key string) (string, any, bool) {
	out, err := exec.Command("hyprctl", "-j", "getoption", "plugin:"+key).Output()
	if err != nil {
		return "", nil, false
	}
...
// right now (only a loaded plugin's values exist), keeping earlier typing when
// the plugin is not loaded so a receipt never loses what it learned.
...
// and the settings detected. Written as <userPluginDir>/<id>.json beside the
// .so and its .abi receipt.
...
// pluginInfo is one row of `hypr plugins list`.
type pluginInfo struct {
	...
	// for a plugin that plays samples: the user's own profile folder (seeded
	// with a README) and the shipped profiles, so customising is one click
	...
	// Current: a copy matches the installed headers, what the next session runs;
	// Status above is about the compositor running now. The doctor converges on
	// this one after an update, before the user logs back in.
...
// pluginEnabled reads a plugin's toggle from the store: bundled plugins have a
// typed section, anything else lives under plugins.extra.
...
// pluginRosterNow builds the full roster against the live compositor. An
// enabled plugin that is installed, not stale and not loaded is loaded here,
// which is what a reload would do, so its row carries the compositor's own
...
func receiptDef(r pluginReceipt) pluginDef {
	...
	docs := r.Repo
	return pluginDef{ID: r.ID, Name: name, Desc: r.Desc, Repo: r.Repo, Plugin: r.Plugin, Local: r.Local, Docs: docs, Loaded: loadedName}
}
...
func describePlugin(d pluginDef, o Overrides, target hyprABI, loaded map[string]loadedPlugin) pluginInfo {
	...
	info := pluginInfo{
		...
		Rebuildable: !nixManagedHyprPlugins() && (d.Repo != "" || d.Local != ""),
		Removable:   !bundled,
		Settings:    []detectedSetting{},
	}
	// a plugin authored here documents itself beside its source: open that copy
	if d.Local != "" {
	...
	case stale:
		...
		if nixManagedHyprPlugins() {
			info.Detail = abiDiff(
				c.ABI,
				target,
			) + ". The running compositor does not match the active NixOS Ryoku generation."
		} else {
			info.Detail = abiDiff(
				c.ABI,
				target,
			) + ". Rebuild it for this Hyprland."
		}
	case info.Enabled && target.ok():
		if err := hyprctlPluginLoad(c.Path); err != nil {
			...
			if mismatchRe.MatchString(err.Error()) {
				...
				if nixManagedHyprPlugins() {
					info.Detail += ". Start a Hyprland session from the active NixOS Ryoku generation."
				} else {
			...
			info.Loaded = true
			info.Status, info.Detail = "running", "Loaded in the running compositor."
			// the plugin came up with its defaults; give it the user's settings
			hyprEval(genPluginConfig(o))
			if !bundled {
				// the name it reports is whatever the loaded list gained: that is
				// the guard key settings.lua needs, so the receipt keeps it
...
// runHyprPlugins dispatches `ryoku-hub hypr plugins <verb>`.
func runHyprPlugins(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("hypr plugins needs list|rebuild|add|remove")
	}
	switch args[0] {
	case "list":
	...
	default:
		return fmt.Errorf("unknown hypr plugins subcommand: %s", args[0])
	}
}
