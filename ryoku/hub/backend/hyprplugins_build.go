...
// hyprplugins_build.go: building a compositor plugin on this box. One builder
// serves every path: deploy.sh on a checkout, the Plugins page's Rebuild after a
// Hyprland bump left a package copy behind, the doctor converging an enabled
// plugin on `ryoku update`, and Add for a plugin from any git repository. The
// recipe is the upstream's own hyprpm.toml (the manifest hyprpm reads): its
// commit_pins pick the commit for the Hyprland being built against and its
// build steps run against the installed headers. The result lands under
...
// tipManifest reads hyprpm.toml as the default branch's tip has it, whatever
// the clone is checked out on: the pin table only grows at the tip, and a
// pinned commit's own copy predates the pins that came after it.
...
// existing clone up to date. Full history, since a pin may point well behind
// the default branch.
...
// checkoutRoot is the repository a dev box deploys from: the flag, then the
// env, then the pointer deploy.sh records.
...
// installSo lays a built .so into the user tier with its .abi receipt. The
// bytes go to a fresh inode (atomicWrite renames into place): truncating a .so
// the running compositor has mapped corrupts its image and crashes it.
func installSo(built, id string, abi hyprABI) (string, error) {
	// drop symbols the way makepkg does; the built file is not mapped by anyone
...
// copyTree replaces dst with a copy of src (regular files and directories;
// .git and build outputs skipped), used to stage a local source and to lay a
// plugin's assets.
...
// buildPlugins runs a set of jobs, sharing one fetch per repository. Progress
// goes to log; the summary is returned for the caller to print as JSON.
func buildPlugins(jobs []buildJob, checkout string, log io.Writer) buildResult {
	...
	for _, j := range jobs {
		...
		case d.Local != "":
			...
			// build a copy: the checkout stays clean and the shipped copy is read-only
		...
		if d.Repo != "" {
			...
			// the pinned commit's own manifest names its build steps and output
...
// needsBuild says whether a plugin has no copy that matches the installed
// headers (what a build would produce). A Ryoku-authored plugin is also stale
// once its source is newer than the local build, so a checkout picks up edits.
...
// runPluginRebuild: `hypr plugins rebuild [--all|--stale] [--checkout <dir>] [<id>...]`.
// Log lines go to stderr, the JSON summary to stdout.
func runPluginRebuild(args []string) error {
	if nixManagedHyprPlugins() {
		return fmt.Errorf(
			"Hyprland plugin binaries are managed declaratively by NixOS; update or rebuild the Ryoku generation instead",
		)
	}
	...
	if !all && !stale && len(ids) == 0 {
		return fmt.Errorf("hypr plugins rebuild needs --all, --stale or plugin ids")
	}
	...
	if len(res.Built) > 0 {
		// re-emit settings.lua so it names the fresh copies, and swap a plugin
		// the compositor is running for its fresh build: the old image stays
		// mapped until it is unloaded, so a rebuild would otherwise take effect
		// only at the next login
		o := loadOverrides()
		_ = writeGeneratedLua(o)
...
// repoInspection is what `add --inspect` reports: the plugins a repository
// offers, so the Hub can let the user choose before anything builds.
...
// runPluginAdd: `hypr plugins add [--inspect] <git-url> [<plugin>...]`.
func runPluginAdd(args []string) error {
	...
	if len(rest) == 0 {
		return fmt.Errorf("hypr plugins add needs a git URL")
	}
	url := rest[0]
	if tc := toolchain(); !tc.OK {
		return fmt.Errorf("cannot build plugins here: missing %s", strings.Join(tc.Missing, ", "))
...
// runPluginRemove: `hypr plugins remove <id>` drops a locally added plugin: its
// copy leaves the compositor, its files leave the user tier, its store entry
// goes. A bundled plugin is turned off, not removed (its package owns it).
func runPluginRemove(args []string) error {
	if len(args) != 1 {
		return fmt.Errorf("hypr plugins remove needs one plugin id")
	}
	id := args[0]
	if _, bundled := bundledByID(id); bundled {
	...
	o := loadOverrides()
	if _, ok := o.Plugins.Extra[id]; ok {
		delete(o.Plugins.Extra, id)
	...
	return printJSON(map[string]string{"removed": id})
}
