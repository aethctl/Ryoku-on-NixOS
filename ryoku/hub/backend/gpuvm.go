...
func vmReadiness() (bool, string, *Capability) {
	if _, err := os.Stat("/etc/NIXOS"); err == nil &&
		!nixGpuPassthroughConfigured() {
		return false,
			"GPU passthrough is disabled on NixOS until the host is configured declaratively by Musubi",
			nil
	}
...
func resolveQEMUEmulator() (string, error) {
	if _, err := os.Stat("/etc/NIXOS"); err == nil {
		const stable = "/run/libvirt/nix-emulators/qemu-system-x86_64"
	...
	if err != nil {
		return "", fmt.Errorf(
			"qemu-system-x86_64 is not available from the active virtualization runtime: %w",
			err,
		)
	}
...
func findVirtioISO() string {
	if p := strings.TrimSpace(os.Getenv("RYOKU_VIRTIO_WIN_ISO")); p != "" {
		if fileExists(p) {
			return p
		}
	}
