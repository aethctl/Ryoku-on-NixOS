...
// NixOS does not run Hub's Arch-owned privileged greeter installer.
// The module exports an immutable helper that performs only the permitted
// mutable SDDM-theme payload update.
func TestGreeterApplyInvocationUsesNixHelper(t *testing.T) {
	...
	program, args, err :=
		greeterApplyInvocation("clockwork/orbital")

	if err != nil {
		t.Fatalf(
			"greeterApplyInvocation: %v",
			err,
		)
	}
	...
	if len(args) != 1 ||
		args[0] != "clockwork/orbital" {
		t.Fatalf(
			"args = %#v, want selected slug only",
			args,
		)
	}
}

func TestGreeterApplyInvocationRejectsRelativeNixHelper(
	t *testing.T,
) {
	t.Setenv(
		"RYOKU_SDDM_THEME_APPLY",
	...
	if _, _, err :=
		greeterApplyInvocation(
			"clockwork/orbital",
		); err == nil {
		t.Fatal(
			"relative Nix helper path should be rejected",
		)
	}
}
