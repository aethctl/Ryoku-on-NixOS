...
Item {
    ...
    // ── the roster ───────────────────────────────────────────────────────────
    property var roster: ({ compositor: {}, toolchain: { ok: true, managed: false, missing: [] }, plugins: [] })
    ...
    readonly property bool managedByNix: pg.roster.toolchain ? pg.roster.toolchain.managed === true : false
    ...
    // ── actions ──────────────────────────────────────────────────────────────
    function rebuild(id) {
        if (pg.busy !== "" || !pg.canBuild || pg.managedByNix) return;
        pg.busy = "rebuild:" + id;
        pg.log = [];
        pg.notice = "";
    ...
    function inspect() {
        ...
        if (pg.busy !== "" || url === "" || !pg.canBuild || pg.managedByNix) return;
        pg.busy = "inspect";
        pg.log = [];
        pg.notice = "";
        pg.inspected = null;
    ...
    function addPicked() {
        ...
        if (pg.busy !== "" || !pg.inspected || ids.length === 0 || pg.managedByNix) return;
        pg.busy = "add";
        pg.log = [];
        pg.notice = "";
    ...
    function statusLabel(p) {
        ...
        case "stale": return pg.managedByNix ? I18n.tr("SESSION MISMATCH") : I18n.tr("NEEDS REBUILD");
    ...
    function sourceLabel(p) {
        ...
        case "package":
            if (pg.managedByNix)
                return I18n.tr("NixOS generation");
            return p.package ? p.package + " " + p.version : p.version;
        case "built":
            return I18n.tr("built here %1").arg(p.version);
        ...
        Column {
            ...
            Item {
                ...
                Row {
                    ...
                    spacing: Tokens.s2
                    Btn {
                        visible: !pg.managedByNix && pg.stale.length > 0
                        compact: true
            ...
            Text {
                visible: pg.loaded && pg.managedByNix
                width: parent.width
                wrapMode: Text.WordWrap
                text: I18n.tr("Plugin binaries are managed declaratively by NixOS. Enable and configure them here; update or rebuild Ryoku to change their compiled generation.")
            ...
            Text {
                visible: pg.loaded && !pg.managedByNix && !pg.canBuild
                width: parent.width
                wrapMode: Text.WordWrap
            ...
            Rectangle {
                ...
                Column {
                    ...
                    Row {
                        ...
                        Btn {
                            visible: !pg.managedByNix && card.p && card.p.rebuildable
                            compact: true
            ...
            Rectangle {
                visible: pg.addOpen && !pg.managedByNix
                width: parent.width
