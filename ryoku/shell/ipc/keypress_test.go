...
func TestStandaloneModifierTapPredicate(t *testing.T) {
	cases := []struct {
		name  string
		event *keypressEvent
		want  bool
	}{
		{"super tap", &keypressEvent{keys: []string{"Super"}, state: "tap"}, true},
		{"ctrl tap", &keypressEvent{keys: []string{"Ctrl"}, state: "tap"}, true},
		{"super chord press", &keypressEvent{keys: []string{"Super", "R"}, state: "pressed"}, false},
		{"plain key tap", &keypressEvent{keys: []string{"A"}, state: "tap"}, false},
		{"super pressed", &keypressEvent{keys: []string{"Super"}, state: "pressed"}, false},
		{"nil", nil, false},
	}
	for _, c := range cases {
		if got := standaloneModifierTap(c.event); got != c.want {
			t.Fatalf("%s: got %v want %v", c.name, got, c.want)
		}
	}
}

// writeKeyEvents lays a finite EV_KEY stream (code, value pairs) into the fake
// device file the reader opens once.
func writeKeyEvents(t *testing.T, path string, events ...[2]uint16) {
	t.Helper()
	raw := make([]byte, 0, linuxInputEventSize*len(events))
	for _, e := range events {
		buf := make([]byte, linuxInputEventSize)
		binary.NativeEndian.PutUint16(buf[16:18], evKey)
		binary.NativeEndian.PutUint16(buf[18:20], e[0])
		binary.NativeEndian.PutUint32(buf[20:24], uint32(e[1]))
		raw = append(raw, buf...)
	}
	if err := os.WriteFile(path, raw, 0o600); err != nil {
		t.Fatal(err)
...
func tapTestRoots(t *testing.T) (sysRoot, devRoot, eventFile string) {
	t.Helper()
	sysRoot = filepath.Join(t.TempDir(), "sys", "class", "input")
	devRoot = filepath.Join(t.TempDir(), "dev", "input")
	capDir := filepath.Join(sysRoot, "event0", "device", "capabilities")
	...
	return sysRoot, devRoot, filepath.Join(devRoot, "event0")
}

// With the visualiser off but taps claimed, the reader runs and the stream
// narrows to standalone modifier taps: the Super tap reaches subscribers and
// the plain key press is filtered out.
func TestKeypressTapClaimsFilterFrames(t *testing.T) {
	sysRoot, devRoot, eventFile := tapTestRoots(t)
	// Super down, Super up (a tap), then A down (a chord member).
	writeKeyEvents(t, eventFile, [2]uint16{125, 1}, [2]uint16{125, 0}, [2]uint16{30, 1})

	topic := newEventTopic()
	sub := topic.subscribe()
	defer topic.unsubscribe(sub)
	manager := newKeypressManager(topic, sysRoot, devRoot, filepath.Join(t.TempDir(), "keypresses.json"))
	manager.setTapClaims(true)
	defer manager.setTapClaims(false)
	...
	for {
		...
		case raw := <-sub.frames:
			...
			if reflect.DeepEqual(frame.Keys, []string{"A"}) {
				t.Fatalf("plain key leaked with visualiser off: %s", raw)
			}
			if reflect.DeepEqual(frame.Keys, []string{"Super"}) && frame.State == "tap" {
				seenSuperTap = true
			}
		case <-deadline:
			if !seenSuperTap {
				t.Fatal("Super tap never published with taps claimed")
			}
			return
		}
	}
...
// Dropping the last claim (visualiser off, taps off) stops the reader and
// publishes the disabled frame; nothing else flows.
func TestKeypressTapClaimsReleaseStopsReader(t *testing.T) {
	sysRoot, devRoot, eventFile := tapTestRoots(t)
	writeKeyEvents(t, eventFile, [2]uint16{30, 1})
	...
	for {
		...
		case <-deadline:
			t.Fatal("release did not publish a disabled frame")
		}
	}
}
