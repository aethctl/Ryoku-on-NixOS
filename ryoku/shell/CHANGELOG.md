...
### Fixed
- **NixOS no longer starts the reactor's Pacman log watcher.** The QS Bar particle stream now respects `RYOKU_UPDATE_BACKEND=nix` instead of tailing `/var/log/pacman.log` on Musubi.
